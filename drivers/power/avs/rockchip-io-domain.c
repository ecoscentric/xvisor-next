...
static int rockchip_iodomain_probe(struct platform_device *pdev)
{
	...
	iod = devm_kzalloc(pdev, sizeof(*iod), GFP_KERNEL);
	if (!iod)
		return -ENOMEM;
	...
	if (parent && parent->of_node) {
		...
		dev_dbg(pdev, "falling back to old binding\n");
		iod->grf = syscon_regmap_lookup_by_phandle(np, "rockchip,grf");
	}

	if (IS_ERR(iod->grf)) {
		dev_err(pdev, "couldn't find grf regmap\n");
...
#else
static struct vmm_driver rockchip_iodomain_driver = {
	.probe   = rockchip_iodomain_probe,
	.name  = "rockchip-iodomain",
	.match_table = rockchip_iodomain_match,
};

static int __init rockchip_iodomain_init(void)
{
	return vmm_devdrv_register_driver(&rockchip_iodomain_driver);
}

static void __exit rockchip_iodomain_exit(void)
{
	vmm_devdrv_unregister_driver(&rockchip_iodomain_driver);
}

VMM_DECLARE_MODULE("Rockchip IO-domain driver",
				   "Heiko Stuebner",
				   "GPL v2",
                   0,
                   rockchip_iodomain_init, rockchip_iodomain_exit);
#endif
